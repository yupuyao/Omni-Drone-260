...
namespace ego_planner
{
  ...
  {
      RCLCPP_INFO(node_->get_logger(), "Received new waypoint goal!");

      Eigen::Vector3d end_wp(msg->pose.position.x, msg->pose.position.y, msg->pose.position.z);

      // Add the new waypoint to the queue
      waypoint_queue_.push(end_wp);

      // If not currently executing a goal, start with this one
      if (!is_executing_queue_ && waypoint_queue_.size() == 1)
      {
          processNextWaypoint();
      }
      else
      {
          RCLCPP_INFO(node_->get_logger(), "Waypoint added to queue. Queue size: %zu", waypoint_queue_.size());
      }
  }

  void EGOReplanFSM::processNextWaypoint()
  {
      ...
      Eigen::Vector3d next_wp = waypoint_queue_.front();
      waypoint_queue_.pop();

      RCLCPP_INFO(node_->get_logger(), "Processing next waypoint. Remaining in queue: %zu", waypoint_queue_.size());

      // Plan trajectory to this waypoint
      init_pt_ = odom_pos_;
  ...
  void EGOReplanFSM::execFSMCallback()
  {
    ...
    case EXEC_TRAJ:
    {
        ...
        else if ((local_target_pt_ - end_pt_).norm() < 1e-3) // close to the global target
        {
            ...
            {
                if (target_type_ == TARGET_TYPE::MANUAL_TARGET && is_executing_queue_)
                {
                    // If we're using the waypoint queue, process the next point
                    if (!waypoint_queue_.empty())
                    {
                        processNextWaypoint();
                    }
