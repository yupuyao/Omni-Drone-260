...
const char* high_rate_topics[] = {
    "sensor_combined",
    "vehicle_attitude",
	"vehicle_odometry"
};

const char* mid_rate_topics[] = {
    "vehicle_local_position",
};

bool is_high_rate_topic(const char* topic_name);
bool is_mid_rate_topic(const char* topic_name);
...
void SendTopicsSubs::init() {
    ...
    const char* topic_name = send_subscriptions[idx].orb_meta->o_name;
    if (is_high_rate_topic(topic_name)) {
    orb_set_interval(fds[idx].fd, 5);
    } 
    else if (is_mid_rate_topic(topic_name)){
    orb_set_interval(fds[idx].fd, 10);
    }
    else {
...
bool is_high_rate_topic(const char* topic_name) {
    for (const auto& high_rate_topic : high_rate_topics) {
        if (strcmp(topic_name, high_rate_topic) == 0) {
            return true;
        }
    }
    return false;
...
bool is_mid_rate_topic(const char* topic_name) {
    for (const auto& mid_rate_topic : mid_rate_topics) {
        if (strcmp(topic_name, mid_rate_topic) == 0) {
            return true;
        }
    }
    return false;
